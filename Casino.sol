/**
 * The contract holds the players's funds and provides state channel functionality.
...
contract CasinoBank is chargingGas{
	...
	/** the balance per player in tokens with 5 virtual decimals */
	...
	*  tokens do not have any decimals, but are represented on this contract with 5 decimals.
	* @param receiver  address of the receiver
	*        numTokens number of tokens to deposit (0 decimals)
	*				 chargeGas indicates if the gas cost is subtracted from the user's token balance
	**/
...
contract Casino is CasinoBank{
  ...
  /**
  * creates a new casino contract.
  * @param authorizedAddress the address which may send transactions to the Casino
  ...
  **/
  function Casino(address authorizedAddress, address tokenContract, uint depositLimit, uint8 kGasPrice) CasinoBank(tokenContract, depositLimit) public{
