...
contract Regulator {
   ...
   event AddContractor (address msgSender,bytes32 msg,uint timestamp);
   event UpdateContractorStatus (address msgSender,bytes32 msg,uint timestamp);
   event LabourContractInitiated (address msgSender,bytes32 msg,uint timestamp);
   event LabourDetailsAccepted (address msgSender,bytes32 msg,uint timestamp);
   ...
   function Regulator() {
      owner = msg.sender;
      LabourContractInitiated(msg.sender,"Mined",block.timestamp);
   }
   ...
   function acceptProposal(uint _id) constant returns (address _clinicalTrial) {
      ...
      LabourDetailsAccepted(msg.sender,"Mined",block.timestamp);
   }
   ...
   function submitCro(bytes32 _name, bytes32 _url) {
      ...
      AddContractor(msg.sender,cro.name,block.timestamp);
   }
   ...
   function changeCroStatus(address _addr, uint8 _status) {
      for(uint32 i=0; i<cros.length; i++) {
         if(cros[i].addr == _addr) {
            ...
            {
               UpdateContractorStatus(msg.sender,"Approved",block.timestamp);
            }
            else
            {
               UpdateContractorStatus(msg.sender,"Rejected",block.timestamp);
            }
            break;
         }
      }
